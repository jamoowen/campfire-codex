...
function recipe(id: string): RecipeRecord {
  return {
    ...
    estimatedHandsOnMinutes: 10,
    timeCategory: 'quick_16_30',
    timeBasis: 'stated',
    handsOnTimeBasis: null,
    sourceTimeNote: null,
    passiveTimeNote: null,
    keyIngredients: ['beans'],
    normalizedKeyIngredientCount: 1,
    under10KeyIngredients: true,
    tenOrFewerKeyIngredients: true,
    sainsburysAvailability: 'high',
...
describe('Worker API', () => {
  ...
  it('does not set an identity cookie and keeps summaries bounded', async () => {
    const response = await SELF.fetch(
      'http://localhost/api/search?pageSize=999',
    );
