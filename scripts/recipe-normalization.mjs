...
export function normalizeRecipe(raw, index) {
  ...
  if (!isRouteSafeRecipeId(id)) {
    throw new Error(
      `Recipe ${id} has an invalid id. IDs must match [A-Za-z0-9._-]{1,100}.`,
    );
  }
  ...
  if (keyIngredients.length === 0) {
    throw new Error(
      `Recipe ${id} key_ingredients must contain at least one non-empty string.`,
    );
  }

  const difficulty = string(raw.difficulty, 'easy');
  if (!allowedDifficulty.has(difficulty))
    throw new Error(`Recipe ${id} has unsupported difficulty: ${difficulty}`);
  const timeCategory = string(
    raw.time_category ?? raw.timeCategory,
    'weeknight_31_60',
  );
  if (!allowedTimeCategory.has(timeCategory))
    throw new Error(
      `Recipe ${id} has unsupported time_category: ${timeCategory}`,
    );
  const availability = string(
    raw.sainsburys_availability ?? raw.sainsburysAvailability,
    'medium',
  );
  if (!allowedAvailability.has(availability))
    throw new Error(
      `Recipe ${id} has unsupported sainsburys_availability: ${availability}`,
    );

  const ingredientCount = integer(
    raw.normalized_key_ingredient_count ?? raw.normalizedKeyIngredientCount,
  ...
  const onePotOrPan = bool(
    raw.one_pot_or_pan ?? raw.onePotOrPan,
    onePot || onePan,
  );
  const singleVessel = bool(
    raw.single_vessel ?? raw.singleVessel,
    onePotOrPan || traybake || dishTypes.includes('slow_cooker'),
  );
  ...
  return {
    ...
    sourceAttributionVerified: bool(
      raw.source_attribution_verified ?? raw.sourceAttributionVerified,
      true,
    ),
    cuisines: list(raw.cuisines),
    proteins: list(raw.proteins),
    dishTypes,
    dietary: list(raw.dietary),
    difficulty,
    estimatedTotalMinutes: integer(
      raw.estimated_total_minutes ?? raw.estimatedTotalMinutes,
      0,
    ),
    estimatedHandsOnMinutes:
      raw.estimated_hands_on_minutes == null &&
      raw.estimatedHandsOnMinutes == null
        ? null
        : integer(
            raw.estimated_hands_on_minutes ?? raw.estimatedHandsOnMinutes,
          ),
    timeCategory,
    timeBasis: string(raw.time_basis ?? raw.timeBasis, 'not_stated'),
    handsOnTimeBasis: nullableString(
      raw.hands_on_time_basis ?? raw.handsOnTimeBasis,
    ),
    sourceTimeNote: nullableString(raw.source_time_note ?? raw.sourceTimeNote),
    passiveTimeNote: nullableString(
      raw.passive_time_note ?? raw.passiveTimeNote,
    ),
    keyIngredients,
    normalizedKeyIngredientCount: ingredientCount,
    under10KeyIngredients: bool(
      raw.under_10_key_ingredients ?? raw.under10KeyIngredients,
      ingredientCount < 10,
    ),
    tenOrFewerKeyIngredients: bool(
      raw.ten_or_fewer_key_ingredients ?? raw.tenOrFewerKeyIngredients,
      ingredientCount <= 10,
    ),
    sainsburysAvailability: availability,
    specialtyIngredients: list(
      raw.specialty_ingredients ?? raw.specialtyIngredients,
    ),
    onePot,
    onePan,
    onePotOrPan,
    ...
    selectionBatch: string(
      raw.selection_batch ?? raw.selectionBatch,
      'default',
    ),
  };
}
