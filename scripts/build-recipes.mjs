...
const sourcePath = path.resolve(root, process.argv[2] ?? "private/recipes.json");
...
if (!Array.isArray(rawRecipes)) {
  throw new Error("Expected a top-level array or an object with a recipes array.");
}
...
const catalog = {
  ...
  dataset: {
    ...
    isDemo: false,
    note:
      "Key ingredients are normalized search terms. Full quantities and methods remain with the canonical source.",
  },
  facets: {
...
const report = {
  ...
  authorCount: authors.size,
  sourceSiteCount: sourceSites.size,
  quick30OrLess: recipes.filter((recipe) => recipe.estimatedTotalMinutes > 0 && recipe.estimatedTotalMinutes <= 30).length,
  under10KeyIngredients: recipes.filter((recipe) => recipe.under10KeyIngredients).length,
...
console.log(`Authors: ${report.authorCount}; source sites: ${report.sourceSiteCount}.`);
