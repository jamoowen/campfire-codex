...
try {
  const { stdout } = await execFileAsync("git", ["ls-files", "--cached"], { cwd: root });
  for (const file of stdout.split("\n").filter(Boolean)) {
    if (forbiddenTrackedPath(file)) leaks.push(`${file} is tracked or staged despite repository privacy rules`);
  }
} catch (error) {
...
for (const file of files) {
  ...
  for (const marker of privateMarkers) {
    if (text.includes(marker)) leaks.push(`${path.relative(root, file)} contains ${marker}`);
  }
}
...
console.log(`Privacy check passed across ${files.length} public/source text files.`);
